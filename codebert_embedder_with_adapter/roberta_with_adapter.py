...
class RobertaWithAdaptersConfig(RobertaConfig):
    def __init__(self, adapter_dim=64, adapt_layer_norm=False, unfreeze_hyper_encoder=False, pad_token_id=1,
...
class RobertaEncoderWithAdapter(RobertaEncoder):
    def __init__(self, config, device):
        ...
        self.layer = nn.ModuleList(
            [EncoderLayerWithAdapter(config, device).to(device) for _ in range(config.num_hidden_layers)])
...
class RobertaWithHyperAdapters(nn.Module):
    ...
    def apply_params_to_adapters(self, generated_params):
        ...
        for p, layer in zip(params, self.model.encoder.layer):
            ...
            layer.adapter_down_layer.weight = p.index_select(dim=0,
                                                             index=torch.LongTensor(range(0, d_model * d_adapter)).to(
                                                                 self.device)).view(d_adapter, d_model).to(self.device)
            layer.adapter_up_layer.weight = p.index_select(dim=0, index=torch.LongTensor(
                range(d_model * d_adapter, d_model * d_adapter * 2)).to(self.device)).view(d_model, d_adapter).to(
                self.device)
            layer.adapter_down_layer.bias = p.index_select(dim=0, index=torch.LongTensor(
                range(d_model * d_adapter * 2, d_model * d_adapter * 2 + d_adapter)).to(self.device)).view(
                d_adapter).to(self.device)
            layer.adapter_up_layer.bias = p.index_select(dim=0, index=torch.LongTensor(
                range(d_model * d_adapter * 2 + d_adapter, d_model * d_adapter * 2 + d_adapter + d_model)).to(
                self.device)).view(d_model).to(self.device)

            if self.config.adapt_layer_norm:
                layer.self_attn_layer_norm.weight.data = layer.self_attn_layer_norm.weight.data + p[
                                                                                                  -2 * d_model: -1 * d_model]
