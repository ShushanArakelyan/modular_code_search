...
class ActionModule_v3_one_input(ActionModule_v1):
    def __init__(self, device, eval=False):
        ...
        self.model1 = torch.nn.Sequential(torch.nn.Linear(dim * 3 + 1, 128),
                                          torch.nn.ReLU(),
...
class ActionModule_v3_two_inputs(ActionModule_v1):
    def __init__(self, device, eval=False):
        ...
        self.model1 = torch.nn.Sequential(torch.nn.Linear(dim * 4 + 2, 128),
                                          torch.nn.ReLU(),
                                          torch.nn.Linear(128, 1)).to(self.device)
        # outputs an embedding
        self.model2 = torch.nn.Sequential(torch.nn.Linear(dim * 3 + embedder.max_seq_length, 128), torch.nn.ReLU(),
    ...
    def forward(self, verb, args, _, precomputed_embeddings):
        ...
        model2_input = torch.cat(
            (verb_embedding, prep1_embedding, prep2_embedding, scores_out.squeeze().unsqueeze(dim=0)), dim=1)
        emb_out = self.model2.forward(model2_input)
        return emb_out, scores_out
