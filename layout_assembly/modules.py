...
#         print(tiled_verb_emb.shape, tiled_prep1_emb.shape, tiled_prep2_emb.shape, code_embeddings[:token_count].shape, scores1.shape, scores2.shape)
        model1_input = torch.cat((tiled_verb_emb, tiled_prep1_emb, tiled_prep2_emb, code_embeddings[:token_count], scores1, scores2), dim=1)
        self.scores_out = self.model1.forward(model1_input)
        model2_input = torch.cat((verb_embedding, prep1_embedding, prep2_embedding), dim=1)
        self.emb_out = self.model2.forward(model2_input)
        return self.emb_out, self.scores_out

#     def backward(self, y, pred, loss_func, op):
...
#         self.op = torch.optim.Adam(self.one_input_module.parameters() + self.two_inputs_module.parameters(), lr=1e-5)
...
#     def backward(self, y):
#         return self.modules[self.num_inputs].backward(torch.LongTensor(y).to(self.device), self.loss_func, self.op)
