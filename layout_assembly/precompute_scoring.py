...
def main(num_negatives, neg_sampling_strategy):
    for file_it in range(6):
        ...
        scores_data_map = np.memmap(f'{data_dir}/memmap_scores_data_{file_it}.npy', dtype='float32', mode='w+', shape=(data_map_count, 512, 1))
        scores_offsets_map = np.memmap(f'{data_dir}/memmap_scores_offsets_{file_it}.npy', dtype='int32', mode='w+', shape=(offsets_count, 1))
