...
class LayoutNet:
    def __init__(self, scoring_module, action_module_facade, device, precomputed_scores_provided=False, finetune_codebert=True):
        ...
        self.verb_embedder = torch.nn.Embedding(5000, 5).to(self.device)
        self.verb_dict = {'UNK': 0}
    ...
    def parameters(self):
        if self.finetune_codebert:
            return chain(self.classifier.parameters(), self.action_module_facade.parameters(),
                         self.verb_embedder.parameters(), embedder.model.parameters())
        else:
            return chain(self.classifier.parameters(), self.action_module_facade.parameters(), 
    ...
    def load_from_checkpoint(self, checkpoint):
        ...
        self.verb_embedder.load_state_dict(models['verb_embedder'])
        self.verb_embedder = self.verb_embedder.to(self.device)
    ...
    def save_to_checkpoint(self, checkpoint):
        ...
        model_dict = {'classifier': self.classifier.state_dict(), 
                      'verb_embedder': self.verb_embedder.state_dict()}
    ...
    def get_verb_embeddings(self, verbs):
        verb_embeddings = torch.zeros(len(verbs), 5).to(self.device)
        for i, v in enumerate(verbs):
            v = v.lower()
            vidx = 0
            if v not in self.verb_dict:
                if len(self.verb_dict) < 5000:
                    vidx = len(self.verb_dict)
                    self.verb_dict[v] = vidx
            else:
                vidx = self.verb_dict[v]
            vidx = torch.LongTensor([vidx]).to(self.device)
            verb_embeddings[i] = self.verb_embedder(vidx)
