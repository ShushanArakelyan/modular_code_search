...
class ActionModule_v1_reduced(ActionModule_v1):
    ...
    def parameters(self):
        return chain(self.model1.parameters(), self.model2.parameters(), self.reduce_linear.parameters())
...
class ActionModule_v1_reduced_one_input(ActionModule_v1_reduced):
    def __init__(self, device, eval=False):
        ...
        self.model1 = torch.nn.Sequential(torch.nn.Linear(dim + reduced_dim + 1, 128),
                                          torch.nn.ReLU(),
                                          torch.nn.Linear(128, 1)).to(self.device)
        # outputs an embedding
        self.model2 = torch.nn.Sequential(torch.nn.Linear(reduced_dim + embedder.max_seq_length, 128),
                                          torch.nn.ReLU(),
...
class ActionModule_v1_reduced_two_inputs(ActionModule_v1_reduced):
    def __init__(self, device, eval=False):
        ...
        self.reduce_linear = torch.nn.Linear(embedder.dim, reduced_dim).to(self.device)
        # outputs a sequence of scores
        self.model1 = torch.nn.Sequential(torch.nn.Linear(reduced_dim * 2 + dim + 2, 128),
                                          torch.nn.ReLU(),
                                          torch.nn.Linear(128, 1)).to(self.device)
        # outputs an embedding
        self.model2 = torch.nn.Sequential(torch.nn.Linear(reduced_dim * 2 + embedder.max_seq_length, 128),
                                          torch.nn.ReLU(),
                                          torch.nn.Linear(128, dim)).to(self.device)
