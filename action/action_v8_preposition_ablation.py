...
class ActionModule_v8_one_input_preposition_ablation(ActionModule_v8_one_input):
    def forward(self, _, arg1, __, precomputed_embeddings):
        ...
        prep_embedding = torch.ones_like(prep_embedding).to(self.device) * 0.5
...
class ActionModule_v8_two_inputs_preposition_ablation(ActionModule_v8_two_inputs):
    def forward(self, _, args, __, precomputed_embeddings):
        ...
        if precomputed_embeddings is None:
            raise ProcessingException()
        prep1_embedding = torch.ones_like(prep1_embedding).to(self.device) * 0.5
        prep2_embedding = torch.ones_like(prep2_embedding).to(self.device) * 0.5
        verb_embedding, code_embeddings = precomputed_embeddings
